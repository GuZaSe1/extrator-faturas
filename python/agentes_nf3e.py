#!/usr/bin/env python3
"""Extrai e valida uma fatura NF3-e com dois agentes locais do Ollama."""
...
CAMINHO_PROMPT_EXTRATOR = DIRETORIO_PROJETO / "prompts" / "fatura_nf3e.txt"
...
def fCarregaConfiguracao(caminho_env: Path = CAMINHO_ENV) -> Configuracao:
    ...
    return Configuracao(
        ...
        modelo_extrator=(
            os.getenv("OLLAMA_EXTRACTOR_MODEL") or "qwen2.5:14b"
        ).strip(),
        modelo_validador=(
...
def fCarregaInstrucoesExtrator() -> str:
    template = fLePrompt(CAMINHO_PROMPT_EXTRATOR)
...
def fCriaAgentes(
...
) -> tuple[AgenteOllama[DadosFatura], AgenteOllama[ResultadoValidacao]]:
    ...
    return agente_extrator, agente_validador
...
def fExecutaFluxo(
    ...
    agentes: tuple[Any, Any] | None = None,
) -> dict[str, Any]:
    ...
    agente_extrator, agente_validador = agentes or fCriaAgentes(configuracao)

    inicio_extrator = perf_counter()
    dados_extraidos = agente_extrator.executar(fMontaEntradaExtrator(texto_aws))
    tempo_extrator = perf_counter() - inicio_extrator
    if not isinstance(dados_extraidos, DadosFatura):
